...
'''
KS.
This code generates and saves latent representations from
the 10-unit fully-connected layer preceding the binary softmax,
for a set of probe images.

NOTE: Need to change ckpt flag and paths in hyper_parameters.py
to match network you want to test.
'''

import os
from resnet import * # make sure matches definition of architecture you're testing
...
# save_name = 'latents_mainset10k'
# max_scene = 9999 # should match whatever we've processed for the PixelVAEs
#
# img_dir = '../../../data/expt3a_modulated_ims/'
# save_name = 'latents_expt3a_modulated_ims'
# max_scene = 500 # should match whatever we've processed for the PixelVAEs
#
# img_dir = '../../../data/expt2a_gloss_ratings/'
# save_name = 'latents_expt2a_gloss_ratings'
# max_scene = 50 # should match whatever we've processed for the PixelVAEs
#
# img_dir = '../../../data/constancy_test_sequences_03/'
# save_name = 'latents_constancy_test_sequences_03'
# max_scene = 1420 # should match whatever we've processed for the PixelVAEs
#
# img_dir = '../../../data/constancy_7a_test_sequences_01/stimuli/'
# save_name = 'latents_constancy_7a_test_sequences_01'
# max_scene = 80 # should match whatever we've processed for the PixelVAEs
#
# img_dir = '../../../data/constancy_4b_test_sequences_01/'
# save_name = 'latents_constancy_4b_test_sequences_01'
# max_scene = 1400 # should match whatever we've processed for the PixelVAEs

img_dir = '../../../data/blender_comparison/interreflection_probe_set/'
save_name = 'latents_blender_interreflection_probe_set'
max_scene = 24 # should match whatever we've processed for the PixelVAEs

# ---------------------------------------------------------------------
test_scene = 0
scene_data = range(0,max_scene) # how many/which of your images you want to use here (CIFAR = 50k train)
train_cutoff = test_scene # i.e. assuming we want to get latents for all images in the directory
...
def prepare_all_data():
    ...
    nb. In this "save_latents" script training labels are never used, so need not be accurate.
    ...
    # im_dir = os.path.join(img_dir, 'rgb/') # may need to switch btwn 'rgb' and 'raw' depending on probes
    im_dir = img_dir
    ...
    # load label data
    # NB: for Probe sequences, this might be irrelevant, e.g. all 999. Still needed for initialising network.

    # Fetch the .csv file that provides information about each of the scenes
    scene_log = pd.read_csv('../../scene_log.csv')
    print(scene_log.head()) # sanity checking

    scene_min = scene_data[0]
    scene_max = scene_data[0]+max_scene
    factors = ['gloss_cat']

    probe_scenes = ['scene'+str(s).zfill(6) for s in range(scene_min, scene_max+1)]
...
class Train(object):
    ...
    def get_latents(self, test_image_array):
        '''
        KS: written to return the latents from a named layer of interest.
        ...
        # prints an exhaustive list of all ops and their types. Useful for finding the name of the one we want.
        # graph = tf.get_default_graph()
        # print([op for op in graph.get_operations()])
        ...
        for step in range(num_batches):
            ...
            units = sess.run('fc/add:0', feed_dict={self.test_image_placeholder: test_image_batch}) # I believe this is the output of the latent FC layer
            print(units.shape)
            latent_list.append(units) # laziness cos I find it easier to manipulate lists

        # KS: Can't get this working so have scrapped for now, just ensure desired probe set is div by test_batch_size
        # # If test_batch_size is not a divisor of num_test_images
        # if remain_images != 0:
        #     # need to reinitialise model here so the input placeholder is the correct size
        #     self.test_image_placeholder = tf.placeholder(dtype=tf.float32, shape=[remain_images,
        #                                                 IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH])
        #     # # Build the test graph
        #     # logits = inference(self.test_image_placeholder, FLAGS.num_residual_blocks, reuse=True)
        ...
        latent_array = np.array([item for sublist in latent_list for item in sublist]) # flatten and turn to npy
...
latent_array = train.get_latents(test_data)
if not os.path.exists(os.path.join(train_dir, 'latent_reps/')):
    os.makedirs(os.path.join(train_dir, 'latent_reps/'))
np.save(os.path.join(train_dir, 'latent_reps/{}.npy'.format(save_name)), latent_array)
